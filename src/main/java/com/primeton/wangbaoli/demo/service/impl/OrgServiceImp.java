...
public class OrgServiceImp implements IOrgService {
	...
	@Override
	public List<Org> queryChildOrgs(Integer superOrgId) throws DemoException {
		ServiceValidator.checkInfoIsNull(superOrgId);
		List<Org> data= orgMapper.queryChildOrgs(superOrgId);
		if (data == null) {
			throw new DemoException(ErrorEnum.ERROR_ORG_ID);
		}
		return data;
