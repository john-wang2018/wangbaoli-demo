...
public class UserServiceImp implements IUserService {
	...
	public User modifyUser(User user, HttpSession session) throws DemoException {
		...
		user.setId(id);
		User objectInfo = userMapper.getUserByUserName(userName);
		if (objectInfo == null) {
			userMapper.updateUser(user);
		} else {
			Integer objectId = objectInfo.getId();
			if (data.getId().equals(objectId)) {
	...
	@Override
	public PageInfo<User> queryUsers(Integer page, Integer size) throws DemoException {
	...
	@Override
	public PageInfo<User> queryByLikeName(String userName, Integer page, Integer size) throws DemoException {
		PageHelper.startPage(page, size);
		String localUserName = "%" + userName + "%";
		User user=new User();
		user.setUserName(localUserName);
		List<User> users = userMapper.queryUsersByKeyWord(user);
		PageInfo<User> pageInfo = new PageInfo<User>(users);
		return pageInfo;
	}
	...
	@Override
	public PageInfo<User> queryUserByOrgId(Integer orgId, Integer page, Integer size) throws DemoException {
		PageHelper.startPage(page, size);
		List<User> users = (List<User>) userMapper.queryUsersByOrgId(orgId);
		PageInfo<User> pageInfo = new PageInfo<User>(users);
		return pageInfo;
	}
